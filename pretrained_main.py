...
if __name__ == "__main__":
    ...
    transformation_images = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=shuffle_valid, drop_last=False,
                              num_workers=num_workers)
    ...
    for j in range(epochs):
        ...
        for i in tqdm(range(len(train_loader))):
            try:
                ...
                if step % eval_every_steps == 0:
                    ...
                    try:
                        ...
                        if use_neptune_monitoring:
                            neptune_monitoring.log_metrics(mode="validation", loss=valid_loss, step=step,
                                                           valid_mode="validation")
                            neptune_monitoring.log_metrics(mode="validation", perplexity=valid_perplexity, step=step,
                                                           valid_mode="validation")
            ...
            #             validation_dict = generate_evaluation_dict(model, valid_loader, tokenizer=tokenizer,
            #                                                        n_eval=refined_validation_batch, device=device)
            #             meteor_score = valid_evaluator.evaluate_meteor(validation_dict)
            #             bleu_score = valid_evaluator.evaluate_bleu(validation_dict, case_sensitive=True, stemming=True,
            #                                                        remove_stopwords=True)["obtained_score"]
            #             rouge_score = valid_evaluator.evaluate_rouge(validation_dict)
            #             if use_neptune_monitoring:
            #                 neptune_monitoring.log_metrics(mode="validation", bleu=bleu_score, step=step,
            #                                                valid_mode="refined")
            #                 neptune_monitoring.log_metrics(mode="validation", rouge=rouge_score, step=step,
            #                                                valid_mode="refined")
            #                 neptune_monitoring.log_metrics(mode="validation", meteor=meteor_score, step=step,
            #                                                valid_mode="refined")
